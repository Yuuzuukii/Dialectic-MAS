"""Overnight sweep (schema 版、全トピック): datasets/ 配下の全トピック x
(protocol loop limit) x (main argument loop limit) を総当たりする。

run_loop_sweep.py は1トピックに対して turns(1,5,10) x attempts(1-10) の30通りを
sweepするが、これを datasets/ 配下の全カテゴリ・全トピックに対して行う。

ログは
  <output_root>/turns{T:02d}_attempts{A:02d}/<category>/<topic>/schema_*.json
...
try:
    from experiments.dialogue.common import LOGS_DIR, all_topic_files, run_schema_topic_once
except ModuleNotFoundError:  # pragma: no cover - direct file execution.
    from common import LOGS_DIR, all_topic_files, run_schema_topic_once  # type: ignore

# プロトコルのループ上限（State.max_turns）の候補。
PROTOCOL_MAX_TURNS = (1, 5, 10)
# 主張のループ上限（1ラウンド・1 proponent あたりの main argument 試行回数の上限）の候補。
MAIN_ARGUMENT_MAX_ATTEMPTS = range(1, 11)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sweep protocol/main-argument loop limits over every topic under datasets/ (schema)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Root directory for sweep logs (default: logs/sweep_all_topics_schema_<timestamp>).",
    )
    parser.add_argument(
        "--turns",
        type=str,
...
async def main() -> None:
    ...
    sweep_root = args.output_root or (LOGS_DIR / f"sweep_all_topics_schema_{started}")
    ...
    print(
        f"=== schema: {len(files)} topics x {len(combos)} combos x {args.runs} runs "
