"""Overnight sweep (mad 版、全トピック): datasets/ 配下の全トピック x
...
ログは
  <output_root>/turns{T:02d}_attempts01/<category>/<topic>/mad_*.json
に保存される。
"""
...
try:
    from experiments.dialogue.common import LOGS_DIR, all_topic_files, run_mad_topic_once
except ModuleNotFoundError:  # pragma: no cover - direct file execution.
    from common import LOGS_DIR, all_topic_files, run_mad_topic_once  # type: ignore
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sweep protocol loop limits over every topic under datasets/ (MAD baseline)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Root directory for sweep logs (default: logs/sweep_all_topics_mad_<timestamp>).",
    )
    parser.add_argument(
        "--only",
        type=str,
...
async def main() -> None:
    ...
    sweep_root = args.output_root or (LOGS_DIR / f"sweep_all_topics_mad_{started}")
    ...
    print(
        f"=== mad: {len(files)} topics x {len(combos)} combos x {args.runs} runs "
