"""Overnight sweep (MAD 版): 1 topic x (protocol loop limit).

相互反論型マルチエージェントディベート(MAD)ベースライン（docs/free_debate_protocol_plan.md）を、
...
ログは
  logs/sweep/<topic_stem>_<開始時刻>/turns{T:02d}_attempts01/scenarios/<topic_stem>/mad_*.json
に保存される。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sweep protocol loop limits for one topic (MAD baseline, mutual rebuttal, no protocol)."
    )
    parser.add_argument(
        "json_file",
